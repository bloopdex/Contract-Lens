...
class SnapshotStoreTest : FunSpec({

    fun surface(name: String): ContractSurface = ContractSurface(
        name = name,
        kind = "openapi",
        formatVersion = "3.0.3",
        operations = listOf(
            Operation(
                ...
                requestBody = null,
                responses = mapOf(
                    "200" to dev.bloopdex.contractlens.core.model.Response(
                        content = mapOf(
                            "application/json" to SchemaNode(
                                NodeType.SCALAR, listOf("string"), null, emptyMap(), emptyList(),
                                null, emptyList(), false, null, null, false, "paths./ping.get.responses.200",
                            )
                        ),
    ...
    test("the index rebuilds by scanning and marks corrupt entries") {
        ...
        Files.writeString(
            s.list().first().path.parent.resolve("garbage@bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.snapshot.json"),
            "not json at all",
        )

        val index = SnapshotStore(s.list().first().path.parent).list() // fresh store instance = index rebuild
    ...
    test("a store path that is a file fails with a store error") {
        ...
        val e = shouldThrow<ContractError.StoreError> { SnapshotStore(file).save(buildSnapshot("x", null, identity, "t", surface("x"))) }
        e.code shouldBe "STORE_ERROR"
    }
})
