...
class GitIdentityTest : FunSpec({
    ...
    test("without an override, a non-git directory fails loudly") {
        val e = shouldThrow<ContractError.GitIdentityUnavailable> {
    ...
    test("without an override, a git repository provides its HEAD sha") {
        ...
        val sha = GitIdentity.require(null, java.nio.file.Path.of("").toAbsolutePath())
