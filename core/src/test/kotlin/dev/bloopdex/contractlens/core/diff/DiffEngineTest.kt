...
private fun surface(vararg ops: Operation): ContractSurface = ContractSurface("test", "openapi", "3.0.3", ops.toList())

private fun jsonResponse(schema: SchemaNode): Map<String, Response> = mapOf("200" to Response(mapOf("application/json" to schema), "r"))

private fun jsonBody(
    schema: SchemaNode,
    required: Boolean = true,
): RequestBody = RequestBody(required, mapOf("application/json" to schema))
...
class DiffEngineTest :
    FunSpec({
        ...
        test("nested property type change reports the precise leaf location") {
            ...
            changes.single().location shouldBe
                "GET /users → response 200 → schema → properties.profile → properties.address → properties.postalCode"
        ...
        test("required-property addition and removal are separate from property add/remove") {
            ...
            kinds(
                DiffEngine.diff(
                    surface(op("get", "/x", responses = jsonResponse(old))),
                    surface(op("get", "/x", responses = jsonResponse(new))),
                ),
            ) shouldBe
            ...
            val s =
                surface(
                    op(
                        "get",
                        "/users",
                        parameters = listOf(param("limit")),
                        responses =
                            jsonResponse(
                                obj(
                                    mapOf("a" to scalar("string")),
                                ),
                            ),
                    ),
                )
        ...
        test("changes are deterministically ordered by location, kind, from, to") {
            ...
            changes.map { it.location } shouldBe
                listOf(
