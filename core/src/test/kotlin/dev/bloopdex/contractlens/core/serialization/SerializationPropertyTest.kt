    ...
    FunSpec({
        ...
        test("canonical JSON round-trips to identical bytes") {
            checkAll(schemaNodeArb(2)) { node ->
                ...
                val reparsed = parseCanonicalJson<SchemaNode>(bytes)
            ...
            checkAll(Arb.int(0, 50)) { n ->
                val node =
                    SchemaNode(
                        ...
                        false,
                        null,
                        Constraints(minLength = n, maxLength = n * 2),
                        false,
                        "c",
                    )
                canonicalJsonBytes(parseCanonicalJson<SchemaNode>(canonicalJsonBytes(node))) shouldBe canonicalJsonBytes(node)
            }
        }
    })
