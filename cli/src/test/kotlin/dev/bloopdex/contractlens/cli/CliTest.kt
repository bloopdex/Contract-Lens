...
class CliTest :
    FunSpec({
        ...
        test("a corrupted snapshot is refused loudly by verify") {
            ...
            val snapshotFile = storeDir.resolve("spec@$sha.snapshot.json")
            // Tamper with the surface but keep the JSON well-formed and
            // the content hash untouched: the integrity check must catch
            // exactly this (a byte-flip that broke JSON validity would be
            // a different, parse-level error).
            val text = Files.readString(snapshotFile)
            Files.writeString(snapshotFile, text.replace("\"spec\"", "\"sped\""))
        ...
        test("a missing contract file is a clear usage error") {
            // mustExist intercepts at parse time (Clikt usage error, exit
            // status 1; main() maps it to the operational exit code 2).
            // The typed FILE_NOT_FOUND path is pinned at the parser level
            // (OpenApiParserTest).
            val result = SnapshotCommand().test(arrayOf("--sha", sha, "no-such-file.yaml"))
            result.statusCode shouldBe 1
            result.stderr shouldContain "no-such-file.yaml"
        }
    })
