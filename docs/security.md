...
model is recorded with the research notes. The automated layer
(SECURITY.md policy, dependency verification, OSV scanning, CI
permissions) sits on top of the manual review below.

## Reviewed surfaces and findings

### File / path handling
- The snapshot store sanitizes contract names before building file
  names (`SnapshotStore.sanitize`) — a contract named
  `../evil` cannot escape the store directory. **Pinned by a
  regression test** (`RobustnessTest`: "a contract name
...
- Schema nesting is bounded by each adapter's depth guard
  (`DEPTH_EXCEEDED` at depth 64) — OpenAPI, GraphQL, and JSON Schema.
- A real-world API dump (~73 KB) is far under the limit; normal use
  is unaffected.

### `$ref` handling
- Local references only; multi-file and remote references are rejected
  with `UNSUPPORTED_REFERENCE`. No network resolution exists and none
  is planned for the core.
...
- All four parsers fail safely and diagnostically on malformed input:
  ...
  third-party exceptions never reach the CLI. Pinned by the seeded
...
- The canonical model never captures descriptions, examples, or other
  raw spec prose, so snapshots and reports cannot leak
  them. **Pinned by a regression test**
...
- stdout carries command output only; structured logs and diagnostics
  go to stderr (logback configuration). Exit codes are the
...
- Spot-checked the two most attack-relevant dependencies (August 2026):
  ...
  - `swagger-parser` 2.1.27 → **UPGRADED to 2.1.40**:
...
- Systematic dependency/supply-chain scanning and checksummed releases
  are covered by the automation section below.

## Automation (implemented 2026-08-19)
