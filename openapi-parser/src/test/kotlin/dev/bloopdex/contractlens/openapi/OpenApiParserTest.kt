...
class OpenApiParserTest : FunSpec({

    fun fixture(relative: String): Path =
        Path.of(requireNotNull(javaClass.getResource("/fixtures/$relative")).toURI())
    ...
    test("parses an OpenAPI 3.0 document into the canonical model") {
        ...
        surface.operations.map { "${it.method} ${it.path}" } shouldBe
            listOf("get /users", "get /users/{id}", "post /users")
        ...
        list.parameters.single { it.name == "limit" }.schema?.constraints?.minimum shouldBe 1.0
        ...
        val userResponse = create.responses.getValue("201").content.getValue("application/json")
        ...
        userResponse.properties.getValue("address").properties.keys shouldBe listOf("city", "street")
        userResponse.properties.getValue("tags").nodeType shouldBe NodeType.ARRAY
        userResponse.properties.getValue("tags").items?.types shouldBe listOf("string")
    ...
    test("parses an OpenAPI 3.1 document with 3.1 features") {
        ...
        list.parameters.single { it.name == "limit" }.schema?.constraints?.minimum shouldBe 0.0

        val user = surface.operations.single { it.path == "/users" && it.method == "post" }
            .responses.getValue("201").content.getValue("application/json")
    ...
    test("terminates recursive refs with REF nodes instead of recursing forever") {
        ...
        val node = surface.operations.single().responses.getValue("200").content.getValue("application/json")
    ...
    test("rejects Swagger 2.0 with the version error, not by converting it") {
        val e = shouldThrow<ContractError.UnsupportedVersion> {
    ...
    test("rejects unresolved references") {
        val e = shouldThrow<ContractError.UnresolvedReference> {
    ...
    test("rejects status keys that collide after normalization") {
        val e = shouldThrow<ContractError.InvalidStructure> {
